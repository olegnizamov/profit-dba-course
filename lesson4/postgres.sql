...
(
    ...
    price     NUMERIC(8, 2),               -- Его цена
    old_price NUMERIC(8, 2) DEFAULT NULL, -- Старая цена (NULL, если цена ранее не снижалась)
...
(
    id    SERIAL,
    title TEXT,
    PRIMARY KEY (id) -- без этого не работает установка внешнего ключа
);
...
ALTER TABLE products ADD category_id BIGINT NULL; -- а postgress ругается на NOT NULL
ALTER TABLE products ADD brand_id BIGINT NULL; -- а postgress ругается на NOT NULL
...
SELECT categories.*, STRING_AGG(DISTINCT brands.title,  ',') AS brand_list
FROM categories
